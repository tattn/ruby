...
#define JIT_IS_PASS(cfp) (VM_FRAME_TYPE(cfp) == VM_FRAME_MAGIC_IFUNC)
...
#define JIT_TRACE \
 { static jit_trace_ret_t ret; jit_trace_insn(th, reg_cfp, reg_pc, &ret); if (ret.jmp == -1) { return ret.retval; } else if (ret.jmp != 1) { RESTORE_REGS();  goto *(void const *)GET_CURRENT_INSN(); } }
